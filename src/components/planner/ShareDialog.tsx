...
export function ShareDialog({ planId, isOpen, onClose, shareToken, shareRole }: ShareDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div
        className="w-full max-w-md glass glass-border rounded-2xl p-6 shadow-card"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-content">Share Plan</h2>
          <button onClick={onClose} className="text-content-secondary hover:text-content" aria-label="Close share dialog">
            X
          </button>
        </div>
        ...
        <div className="mb-4 flex gap-2 pb-2">
          <button
            className={`px-3 py-1 text-sm font-bold uppercase tracking-widest rounded-full transition-colors ${tab === "invite" ? "gradient-brand text-content-inverse shadow-glow" : "text-content-muted"}`}
            onClick={() => setTab("invite")}
          >
            Invite People
          </button>
          <button
            className={`px-3 py-1 text-sm font-bold uppercase tracking-widest rounded-full transition-colors ${tab === "link" ? "gradient-brand text-content-inverse shadow-glow" : "text-content-muted"}`}
            onClick={() => setTab("link")}
          >
            Share Link
          </button>
          ...
          <div>
            <div className="mb-3 flex gap-2">
              <input
                ...
                className="flex-1 rounded-xl border border-surface-border bg-surface-overlay px-3 py-1.5 text-sm text-content placeholder-content-muted focus:outline-none focus:glow-ring"
              />
              <select
                value={role}
                onChange={(e) => setRole(e.target.value as CollaboratorRole)}
                className="rounded-xl border border-surface-border bg-surface-overlay px-2 py-1.5 text-sm text-content"
              >
            ...
            {error && <p className="mb-2 text-xs text-danger-light">{error}</p>}

            {/* Collaborator list */}
            <div className="max-h-48 space-y-2 overflow-y-auto">
              {collaborators.map((collab) => (
                <div key={collab.id} className="flex items-center justify-between rounded-xl bg-surface-overlay px-3 py-2 border-b border-surface-border">
                  <div>
                    <span className="text-sm text-content">{collab.email ?? "User"}</span>
                    <span className="ml-2 rounded bg-surface-border px-1.5 py-0.5 text-[10px] text-content-secondary">
                      {collab.role}
                    </span>
                    {!collab.acceptedAt && (
              ...
              {collaborators.length === 0 && (
                <p className="text-center text-xs text-content-muted">No collaborators yet</p>
              )}
            </div>
          </div>
        ) : (
              ...
              <div className="space-y-2">
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 rounded-xl border border-surface-border bg-surface-overlay px-3 py-1.5 text-xs font-mono text-content-secondary"
                  />
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs text-content-secondary">Role for link users:</span>
                  <select
                    ...
                    className="rounded-xl border border-surface-border bg-surface-overlay px-2 py-1 text-xs text-content"
                  >
                    <option value="editor">Editor</option>
                    <option value="viewer">Viewer</option>
                  </select>
